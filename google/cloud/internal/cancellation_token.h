 ...
 public:
  ...
  void Cancel() {
    cancelled_.store(true);
    {
      ...
      for (auto const& [transfer, wakeup] : wakeups_) wakeup();
    }
    cv_.notify_all();
  }
  ...
  /**
   * Sleeps for @p duration, returning as soon as the token is cancelled.
   *
   * Retry loops back off between attempts. An attempt that is cancelled while
   * backing off would otherwise hold its thread for the rest of the delay,
   * which can be minutes, so they wait here instead of on the clock alone.
   *
   * Returns whether the token was cancelled.
   */
  bool WaitFor(std::chrono::milliseconds duration) {
    std::unique_lock<std::mutex> lock(mu_);
    cv_.wait_for(lock, duration, [this] { return cancelled_.load(); });
