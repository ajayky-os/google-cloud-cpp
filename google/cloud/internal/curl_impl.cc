...
void CurlImpl::RegisterWakeup() {
  ...
  // immediately. Without it a cancellation is still detected, but only at the
  // next WaitForHandles() timeout.
...
void CurlImpl::UnregisterWakeup() {
  ...
  // woken up by a cancellation of this one. `RemoveWakeup()` takes the token's
  // lock, so it also waits out a `Cancel()` that is running concurrently.
