 ...
 * soon as the race is decided, so they release their thread, connection and
 * hedge slot right away instead of running on until a stall timeout, and are
 * closed when they complete.
 *
...
class HedgedObjectReadSource : public ObjectReadSource {
 public:
   ...
   * is known. The child's transfers, including any reconnects it makes later,
   * must honor @p cancel: the source cancels an attempt as soon as it loses a
   * race.
   */
  using ChildFactory =
 ...
 private:
  ...
  // The token `active_child_` was opened with. Kept so that when the active
  // child becomes the primary attempt of a later race and loses it, that
  // attempt can be cancelled. Never cancelled while the child is the winner.
